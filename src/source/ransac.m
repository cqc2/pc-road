...
% INPUT:
% pointData - 待拟合数据，可以是多维点云数据，但拟合只使用前两维
% shapeType - 拟合类型，1一次二维曲线、2二次二维曲线、3三次二维曲线、'plane'
%             平面拟合、'circle'圆拟合
% deviation - 允许偏离量
% permitIterations - 最大迭代次数
%
% OUTPUT:
% coefficients - 拟合结果的系数
% percet - 在偏移量内的元素占总体比例
% pointIndex - 在偏移量内的元素索引
...
while nSatisfiedPoint < nPoint*2/3 &&  iterations<permitIterations,      %有2/3的数据符合拟合模型或达到最大迭代次数就可以退出了
    switch shapeType
        case 1
            [nSatisfiedPoint,coefficients,pointIndex]=  ransacline(pointData,deviation);%一次二维曲线
        case 2
        ...
        case 'circle'
            [nSatisfiedPoint,coefficients]=  ransaccircle(pointData,deviation);%圆拟合
        otherwise
            return;
    end
