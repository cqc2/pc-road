...
for i=1:size(breakLines,2)
    ...
    for m = 1:size(idx,2)
        ...
        figure(3); plot([c1(1) c2(1)],[c1(2) c2(2)],'-','Color',[0 0 0]);hold on;axis equal
...
for iLinked = 1:nLinked
    ...
    range = breakFragmentinfo(iLinked).range;
    width = breakFragmentinfo(iLinked).width;
    length = breakFragmentinfo(iLinked).length;
    if size(linekedline,2)<10||range<60||width>0.2||length<0.7
        continue;
    end
    n = n+1;
...
%     end
    while(loopflag)
        ...
        for iNearst = 2:size(Idx,2) %最近的是自己，所以从2开始
            ...
            searchAngle = 3;%搜寻角，即在在距当前矩形方向一定范围内搜索
            limitAngle1 = abs(preA-pre_now_angle);%连接线夹角
            limitAngle2 = abs(nowA-pre_now_angle);
            if limitAngle1>90
                limitAngle1 = abs(limitAngle1-180);
